#!/usr/bin/env perl
#   $Source: /var/local/cvs/gasnet/ofi-conduit/contrib/gasnetrun_ofi.pl,v $
#     $Date: 2013/08/24 06:21:14 $
# $Revision: 1.14 $
# Description: GASNet VAPI, IBV, MXM, OFI spawner
...
my $spawner = $ENV{'GASNET_OFI_SPAWNER'};
my $conduit = $ENV{'GASNET_OFI_CONDUIT'};
...
# Validate flags
    ...
    if (($spawner eq 'MPI') && !$ENV{GASNET_OFI_BOOTSTRAP_MPI}) {
...
# Run it which ever way makes sense
    ...
    $ENV{'GASNET_OFI_SPAWNER'} = lc($spawner);
    if ($spawner eq 'MPI') {
