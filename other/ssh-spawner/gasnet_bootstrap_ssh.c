...
static void do_master(const char *spawn_args, int *argc_p, char ***argv_p) {
  ...
  { int done = 0;
	...
	break;
	  ...
	  break;
        case BOOTSTRAP_CMD_SNBCAST:
	  gasneti_bootstrapSNodeBroadcast_ssh(NULL, 0, NULL, -1);
	  break;
...
void gasneti_bootstrapSNodeBroadcast_ssh(void *src, size_t len, void *dest, int rootnode_arg) {
  ...
  if (is_master) {
    ...
    for (j = 0; j < children; ++j) {
      gasneti_free(tmp[j].data);
    }
    gasneti_free(tmp);
  } else {
    char cmd = BOOTSTRAP_CMD_SNBCAST;
    struct iovec iov[4];
    iov[0].iov_base = &cmd;
    iov[0].iov_len  = sizeof(cmd);
