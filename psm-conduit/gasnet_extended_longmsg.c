/*
 * Description: PSM long message protocol
...
static psm_mq_req_t *gasnete_get_mq_req(void)
{
    if (gasnetc_psm_state.posted_reqs_length ==
        ...
        gasnetc_psm_state.posted_reqs =
            gasneti_realloc(gasnetc_psm_state.posted_reqs,
                    new * sizeof(psm_mq_req_t));

        gasnetc_psm_state.posted_reqs_alloc = new;
        for (i = old; i < new; i++)
            gasnetc_psm_state.posted_reqs[i] = PSM_MQ_REQINVALID;
    }
...
int gasnete_long_msg_init(void)
{
    ...
    gasnetc_psm_state.posted_reqs = gasneti_malloc(
            sizeof(psm_mq_req_t) * GASNETE_MQREQS_INIT);
...
void gasnete_post_pending_mq_ops(void)
{
    ...
    for (cur = head; cur != NULL; ) {
        psm_mq_req_t *req = gasnete_get_mq_req();
        gasnete_mq_op_t *op = (gasnete_mq_op_t *)cur;

        if (op->type == GASNETE_MQ_SEND_OP) {
            ret = psm_mq_isend2(gasnetc_psm_state.mq, op->peer,
                    0, &op->tag, op->buf,
                    op->length, NULL, req);
        } else {
            psm_mq_tag_t tagsel;
            ...
            ret = psm_mq_irecv2(gasnetc_psm_state.mq, op->peer,
...
void gasnete_finish_mq_reqs(void)
{
    ...
    for (i = 0; i < gasnetc_psm_state.posted_reqs_length; i++) {
        gasneti_assert(gasnetc_psm_state.posted_reqs[i] !=
                PSM_MQ_REQINVALID);
        ret = psm_mq_test2(&gasnetc_psm_state.posted_reqs[i], &stat);
        if (ret != PSM_OK)
            /* No completion */
            continue;
    ...
    psm_amarg_t *args, int nargs, void *addr, uint32_t len)
{
    ...
    psm_am_get_source(token, &op->peer);
        ...
        size_t nbytes, gasnet_handle_t op GASNETE_THREAD_FARG)
{
    ...
    while (nbytes > 0) {
        ...
        ret = psm_am_request_short(epaddr,
        ...
        ret = psm_mq_isend2(gasnetc_psm_state.mq, epaddr, 0, &tag, (void *)srcptr,
...
fail:
    GASNETC_PSM_UNLOCK();
    gasneti_fatalerror("psm_mq_isend2 failure: %s\n",
               psm_error_get_string(ret));
}
...
void gasnete_get_long (void *dest, gasnet_node_t node, void *src,
    ...
    while (nbytes > 0) {
        ...
        ret = psm_mq_irecv2(gasnetc_psm_state.mq, epaddr, &tag, &tagsel,
...
fail:
    GASNETC_PSM_UNLOCK();
        gasneti_fatalerror("psm_mq_irecv2 failure: %s\n",
                psm_error_get_string(ret));
}
